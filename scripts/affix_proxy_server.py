...
localhost_name = "proxy_affix.zenodotus.cs.washington.edu"
...
def start_server(myip, myport):
    ...
    if myport == affix_port:
        print "Listening for connection on %s:%d" % (localhost_name, myport)
        print "Listening with affix_stack: " + affix_string
        affix_obj = AffixStackInterface(affix_string)
        tcp_server_socket = affix_obj.listenforconnection(myip, myport)
    else:
...
def tcp_file_server_incoming(remoteip, remoteport, sockobj):
    ...
    def _handle_incoming_connection():
        ...
        while True:
            try:
                if server_port == affix_port:
                    print "Creating outgoing connection with affix_stack: " + affix_string
                    affix_obj = AffixStackInterface(affix_string)
                    server_sockobj = affix_obj.openconnection(server_address, server_port, localhost_name, random.randint(4000,60000), 10)
                else:
...
def main():
    ...
    if affix_port != -1:
        if len(sys.argv) < 7:
            print "If affix_port is provided (not -1), then affix_string must also be provided."
            usage()
        affix_string = sys.argv[6]
                         
    print "Starting affix proxy"
...
def usage():
    message = "Usage:\n\tpython affix_apache_client.repy server_addr server_port listening_addr listening_port affix_port [affix_string]"
    print message
    sys.exit()
